      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="mt-5 mb-8">
          <div className="flex flex-wrap items-center gap-2 mb-3">
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-2xl shadow-sm border bg-purple-100 border-purple-200`}>{icon}</div>
          ...
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3">
            <div className="flex-1 min-w-0">
              <h1 className="page-title calc-page-h1 text-2xl sm:text-3xl md:text-4xl font-black text-gray-900" style={{fontFamily:"'Inter', system-ui, sans-serif"}}>{displayTitle}</h1>
                ...
                {resolvedRelatedCalculators.map(rc => (
                  ...
                  >
                    ...
                    <span className="block">
                      <span className={`font-semibold text-sm text-gray-800 transition-all block group-hover:text-purple-600`}>{rc.name}</span>
                      <span className="text-xs text-gray-400 mt-0.5 leading-relaxed block">{rc.desc}</span>
                    </span>
                    <ArrowRight className={`w-4 h-4 text-gray-300 transition-all ml-auto flex-shrink-0 mt-0.5 group-hover:text-purple-400`} />
                  </Link>
                ))}
              </div>
            </div>
        ...
        {!blogSlug && (
          <div className="mt-10 text-center">
            <Link href="/blog" className="inline-flex items-center gap-2 text-sm text-blue-600 hover:text-blue-700 font-semibold border border-blue-200 px-4 py-2 rounded-xl bg-blue-50 hover:bg-blue-100 transition-all">
              <BookOpen className="w-4 h-4" /> Browse our ZIP Code Guides <ArrowRight className="w-3 h-3" />
            </Link>
          </div>
        )}
      </div>
