...
export function calculateGeneticHeightPotential(fatherHeightCm: number, motherHeightCm: number, currentHeightCm: number, currentAgeYears: number, gender: 'male' | 'female', nutritionAdequate: boolean, chronicIllness: boolean) {
  ...
  return { targetHeight: Math.round(targetHeight), rangeMin: range.min, rangeMax: range.max, currentHeightCm, remainingGrowthCm: remainingGrowth, growthComplete, modifiers, heightPercentileEst, note: 'Mid-parental height calculation (Tanner method) — an estimate of familial target height, not a prediction of exact adult height.' }
}
...
export function calculateHydrationForExercise(bodyWeightKg: number, exerciseDurationMin: number, intensityLevel: number, temperatureCelsius: number, altitudeFt: number, exerciseType: 'cardio' | 'strength' | 'outdoor') {
  ...
  const sportsDrinkTip = electrolytesNeeded ? 'Consider a sports drink with electrolytes instead of plain water for sessions over 60 minutes or in high heat.' : 'Plain water is sufficient for this session length and intensity.'
