      ...
      {result && (
        <div className="rounded-2xl border p-6" style={{background:'rgba(240,253,244,0.7)',borderColor:'rgba(187,247,208,0.6)'}}>
          <div className="text-center mb-4">
            <div className="text-5xl font-black text-green-600 font-mono mb-1">{result.zip}-0000</div>
            <div className="text-gray-500">{result.city}, {result.state}</div>
          </div>
          <div className="p-3 rounded-xl bg-blue-50 border border-blue-200 text-sm text-blue-700">
            💡 The +4 code identifies a specific delivery point. For the exact +4, use the official USPS lookup tool at usps.com.
          </div>
        </div>)}
    </div>
  )
