      ...
      {result&&(
        <div className="rounded-2xl border p-6" style={{background:'rgba(240,253,244,0.7)',borderColor:'rgba(187,247,208,0.6)'}}>
          <div className="text-center mb-5">
            <div className="text-6xl font-black text-green-600 mb-1">{result.stateCode}</div>
            <div className="text-2xl font-bold text-gray-800 mb-1">{result.state}</div>
            <div className="text-gray-500">ZIP {result.zip} — {result.city}</div>
          </div>
          <div className="grid grid-cols-2 gap-3 mb-5">
            ...
            <div className="rounded-xl border p-3 bg-white/70 text-center">
              ...
              <div className="font-black text-gray-900 text-sm">{result.county}</div>
            </div>
          </div>
          <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(result.state)}`}
            target="_blank" rel="noopener noreferrer"
            className="w-full flex items-center justify-center gap-2 px-5 py-2.5 text-white font-semibold rounded-xl"
            style={{background:'linear-gradient(135deg,#3b82f6,#2563eb)'}}>
            🗺️ View {result.state} on Google Maps
          </a>
        </div>
      )}
    </div>
