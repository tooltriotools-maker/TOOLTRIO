...
const seoContent = {
  ...
  featureCards: [
    ...
    { icon: '☀️', title: `DST Aware`, desc: `Knows which states observe DST and which don't (AZ, HI, territories are permanent standard time).`, bullets: [] },
...
Timezone boundaries in the US generally follow state lines, but in several states the boundary runs through the state, creating splits. Indiana ZIP codes fall in both ET and CT depending on county — most of the state is ET, but southwestern Indiana (around Evansville) is CT. Tennessee is split between ET (east) and CT (west). Kentucky is mostly ET but western counties are CT. Florida's Panhandle west of the Apalachicola River is CT while the rest of Florida is ET. Our tool uses per-ZIP-code timezone assignments to correctly handle all these edge cases.
...
Call center teams use ZIP timezone data to determine when to call prospects — a lead in a 213 area code in Los Angeles should not be called at 8 AM when a rep in New York is starting their day, because it would be 5 AM Pacific time. Automated systems that send appointment reminders, shipping notifications, and promotional messages use ZIP timezone to localize the send time to each recipient's local morning or afternoon, improving open rates and reducing opt-outs.
