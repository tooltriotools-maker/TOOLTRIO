...
function elevationLabel(ft: number) {
  if (ft < 0)    return { label: 'Below Sea Level', color: 'text-blue-600',  icon: '🌊' }
  if (ft < 500)  return { label: 'Lowland',          color: 'text-green-600', icon: '🌾' }
  if (ft < 1500) return { label: 'Low Elevation',    color: 'text-lime-600',  icon: '🌿' }
  if (ft < 3500) return { label: 'Mid Elevation',    color: 'text-yellow-600',icon: '⛰️' }
  if (ft < 7000) return { label: 'High Elevation',   color: 'text-orange-600',icon: '🏔️' }
  return                 { label: 'Alpine',           color: 'text-red-600',   icon: '🗻' }
}
    ...
    <div>
      ...
      <div className="flex gap-2 mb-6">
        <input value={zip} onChange={e => setZip(e.target.value.replace(/\D/g, ''))} onKeyDown={e => e.key === 'Enter' && lookup()}
          placeholder="Enter ZIP code (e.g. 80202)" maxLength={5}
      ...
      {result && elev && (
        <div className="rounded-2xl border p-6" style={{ background: 'rgba(240,253,244,0.7)', borderColor: 'rgba(187,247,208,0.6)' }}>
          <div className="text-center mb-5">
            <div className="text-5xl mb-1">{elev.icon}</div>
            <div className={`text-5xl font-black mb-1 ${elev.color}`}>{(result.elevation ?? 0).toLocaleString()} <span className="text-2xl">ft</span></div>
            <div className="text-lg font-semibold text-gray-600">{Math.round((result.elevation ?? 0) * 0.3048).toLocaleString()} meters</div>
            <div className={`text-base font-bold mt-1 ${elev.color}`}>{elev.label}</div>
          </div>
