      ...
      {result && (
        <div className="rounded-2xl border p-6" style={{background:'rgba(240,253,244,0.7)',borderColor:'rgba(187,247,208,0.6)'}}>
          <div className="text-center mb-4">
            <div className="text-5xl font-black text-green-600 mb-1">{result.population>0?result.population.toLocaleString():'N/A'}</div>
            <div className="text-gray-600">people in ZIP {result.zip}</div>
            <div className="text-sm text-gray-400 mt-1">{result.city}, {result.state}</div>
          </div>
          {result.population===0&&<div className="text-center text-amber-600 text-sm mt-2">PO Box or non-delivery ZIP — no resident population data</div>}
        </div>)}
    </div>
  )
}
