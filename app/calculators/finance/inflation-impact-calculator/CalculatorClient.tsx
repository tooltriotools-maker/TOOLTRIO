'use client'
import { calculateInflationImpact } from '@/lib/calculations/finance'
...
export default function CalculatorClient({faqs,relatedCalculators}:Props) {
  ...
  const result = useMemo(()=>{
    try{return calculateInflationImpact(currentAmount, inflationRate, years, investmentReturn)}catch(e){return null}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-1 h-fit space-y-3">
          ...
          <div className="space-y-1">
            <label className="text-xs font-medium text-gray-600">Current Amount ($)</label>
            <div className="flex items-center gap-2 border rounded-xl px-3 py-2" style={{background:'rgba(248,250,248,0.8)',borderColor:'rgba(226,232,240,0.7)',backdropFilter:'blur(6px)'}}>
              <span className="text-green-600 text-sm">$</span>
              <input type="number" value={currentAmount} onChange={e=>setCurrentAmount(Number(e.target.value))} step={1000} className="bg-transparent text-gray-900 font-semibold w-full outline-none text-right" />
          ...
          {result ? (
            <>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                <ResultCard label="Future Cost of Same Goods" value={result ? `${Number(result.futureValue).toLocaleString(undefined,{maximumFractionDigits:0})}` : "—"} highlight />
                <ResultCard label="Purchasing Power Loss" value={result ? `${Number(result.purchasingPowerLoss).toLocaleString(undefined,{maximumFractionDigits:0})}` : "—"} />
                <ResultCard label="Invested Value" value={result ? `${Number(result.investedValue).toLocaleString(undefined,{maximumFractionDigits:0})}` : "—"} />
                <ResultCard label="Real Return" value={result ? `${Number(result.realReturn).toFixed(1)}%` : "—"} />
                <ResultCard label="Real (Inflation-Adj) Value" value={result ? `${Number(result.realValue).toLocaleString(undefined,{maximumFractionDigits:0})}` : "—"} />
              </div>
