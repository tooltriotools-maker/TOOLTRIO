'use client'
import { calculateGoldVsStocks } from '@/lib/calculations/finance'
        ...
        <Card className="lg:col-span-1 h-fit space-y-3">
          ...
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-2 border rounded-xl px-3 py-2" style={{background:'rgba(248,250,248,0.8)',borderColor:'rgba(226,232,240,0.7)'}}>
              ...
              <input type="number" value={goldAmount} onChange={e=>setGoldAmount(Number(e.target.value))} step={5000} className="bg-transparent text-gray-900 font-semibold w-full outline-none text-right" />
              
            </div>
          </div>
          <div className="space-y-1">
            <label className="text-xs font-medium text-gray-600">Stock Allocation (%)</label>
            <div className="flex items-center gap-2 border rounded-xl px-3 py-2" style={{background:'rgba(248,250,248,0.8)',borderColor:'rgba(226,232,240,0.7)'}}>
          ...
          {result ? (
            <>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                <ResultCard label="Portfolio (blended)" value={result ? `${Number(result.blendedFinal).toLocaleString(undefined,{maximumFractionDigits:0})}` : "—"} highlight />
