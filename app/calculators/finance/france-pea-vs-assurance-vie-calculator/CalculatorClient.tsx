...
export default function CalculatorClient({ faqs, structuredData, relatedCalculators, blogSlug }: Props) {
  ...
  return (
    <CalculatorLayout title="France PEA vs Assurance-Vie Calculator 2026" description="Compare Plan Epargne en Actions vs Assurance-Vie on tax efficiency and long-term wealth building." icon="🇫🇷" category="Finance" structuredData={structuredData} relatedCalculators={relatedCalculators} blogSlug={blogSlug}>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-1 h-fit">
          ...
          <div className="space-y-4">
            ...
            <InputField label="PEA Return (p.a.)" value={rateA} onChange={setRateA} min={1} max={20} step={0.25} suffix="%" />
            <InputField label="Assurance-Vie Return (p.a.)" value={rateB} onChange={setRateB} min={1} max={15} step={0.25} suffix="%" />
          ...
          <div className="mt-3 grid grid-cols-2 gap-2 text-xs">
            <div className="p-2 bg-green-900/20 rounded-lg text-center border border-green-800/30">
              <p className="text-green-400 font-bold">PEA</p>
            ...
            <div className="p-2 bg-blue-900/20 rounded-lg text-center border border-blue-800/30">
              <p className="text-blue-400 font-bold">Assurance-Vie</p>
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <ResultCard label="PEA Value" value={fmtC(result.fvA)} subValue={`Gain: ${fmtC(result.gainA)}`} highlight={result.aBetter} icon={<TrendingUp className="w-4 h-4" />} />
            <ResultCard label="Assurance-Vie Value" value={fmtC(result.fvB)} subValue={`Gain: ${fmtC(result.gainB)}`} highlight={!result.aBetter} icon={<Shield className="w-4 h-4" />} />
            <ResultCard label="Total Invested" value={fmtC(result.invested)} subValue={`${years}yr x €${monthly}/mo`} />
            <ResultCard label="Advantage" value={fmtC(result.diff)} subValue={result.aBetter ? 'PEA wins' : 'Assurance-Vie wins'} highlight />
          </div>
          <Card>
            <h3 className="text-sm font-semibold text-gray-700 mb-4">PEA vs Assurance-Vie - Wealth Growth Over {years} Years</h3>
            <div style={{ height: 250 }}>
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={result.yearlyData} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                  <defs>
                    <linearGradient id="gOptA_france-pea-vs-assurance-vie-calculator" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#10b981" stopOpacity={0.4} /><stop offset="95%" stopColor="#10b981" stopOpacity={0} /></linearGradient>
                    <linearGradient id="gOptB_france-pea-vs-assurance-vie-calculator" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#3b82f6" stopOpacity={0.4} /><stop offset="95%" stopColor="#3b82f6" stopOpacity={0} /></linearGradient>
                  </defs>
                  ...
                  <Area type="monotone" dataKey="optB" name="Assurance-Vie" stroke="#3b82f6" fill="url(#gOptB_france-pea-vs-assurance-vie-calculator)" strokeWidth={2} />
                  <Area type="monotone" dataKey="optA" name="PEA" stroke="#10b981" fill="url(#gOptA_france-pea-vs-assurance-vie-calculator)" strokeWidth={2.5} />
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </Card>
          <Card>
            ...
            <div className="overflow-y-auto max-h-52">
              <table className="calc-table">
                <thead><tr><th>Year</th><th>Invested</th><th>PEA</th><th>Assurance-Vie</th><th>Advantage</th></tr></thead>
                <tbody>
        ...
        <div className="rounded-3xl border p-6 md:p-8" style={{background:'rgba(255,255,255,0.82)',backdropFilter:'blur(12px)',borderColor:'rgba(226,232,240,0.7)',boxShadow:'0 8px 30px rgba(15,23,42,0.06)'}}>
          <h2 className="text-xl md:text-2xl font-black text-gray-900 mb-6" style={{fontFamily:"'Playfair Display', serif"}}>France Pea vs Assurance Vie: Complete Guide</h2>
      ...
      <Card className="mt-6">
        <h2 className="text-lg font-black text-gray-900 mb-3">
          France PEA vs Assurance-Vie Example (France 2026)
        </h2>
        <p className="text-sm text-gray-600">
          For example, investing EUR 100,000 over 15 years in France, your PEA vs Assurance-Vie calculator 2026 shows the after-tax wealth difference between both accounts based on your withdrawal strategy.
        </p>
      </Card>

            <SEOContent
        title="France PEA vs Assurance-Vie Calculator – Which French Investment Account Is Best in 2026?"
        category="finance"
...
Assurance-Vie is more flexible in investment options (you can hold a mix of fonds en euros providing guaranteed returns and unités de compte for equity/bond exposure) and has more favorable inheritance tax treatment. After 8 years, withdrawals of gains are taxed at a reduced 7.5% flat rate (for contracts up to €150,000 total) versus the standard 30% prélèvement forfaitaire unique.`}
...
Fonds en euros within Assurance-Vie: Capital-guaranteed funds currently yielding 2.5-3.5% depending on insurer. Suitable for risk-averse portion of portfolio while equity unités de compte handle growth. The capital guarantee makes Assurance-Vie particularly useful for funds you'll need in 5-10 years.`}
        benefits={[
        ...
        tipsSection={`Open an Assurance-Vie as early as possible regardless of contribution amount — the 8-year clock for favorable tax treatment starts at contract opening, not at the first major contribution. Opening with minimum amounts (often €100-500) and contributing more later is the smart approach.
...
For inheritance planning specifically, Assurance-Vie is one of the most powerful tools in France: amounts up to €152,500 per beneficiary can be passed outside of the normal inheritance tax regime, making it central to estate planning for many French families.`}
