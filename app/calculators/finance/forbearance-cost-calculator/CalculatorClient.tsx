...
export default function CalculatorClient({faqs,relatedCalculators}:Props) {
  ...
  const [resumePayment, setResumePayment] = useState(2200)
  ...
  return (
    <CalculatorLayout title="Mortgage Forbearance Cost Calculator USA 2026" description="Calculate the true long-term cost of taking mortgage forbearance — interest accrued, payment increases, and total additional cost vs continuing payments." icon="🏠" category="Finance" structuredData={} relatedCalculators={relatedCalculators} slug="forbearance-cost-calculator">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-1 h-fit space-y-3">
          ...
          <div className="space-y-1">
            <label className="text-xs font-medium text-gray-600">Current Payment ($)</label>
            <div className="flex items-center gap-2 border rounded-xl px-3 py-2" style={{background:'rgba(248,250,248,0.8)',borderColor:'rgba(226,232,240,0.7)'}}>
              <span className="text-green-600 text-sm">$</span>
              <input type="number" value={resumePayment} onChange={e=>setResumePayment(Number(e.target.value))} step={50} className="bg-transparent text-gray-900 font-semibold w-full outline-none text-right" />
