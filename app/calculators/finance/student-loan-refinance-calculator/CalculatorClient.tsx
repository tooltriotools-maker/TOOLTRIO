...
export default function CalculatorClient({faqs,relatedCalculators}:Props) {
  ...
  const result = useMemo(()=>{
    try{return calculateStudentLoanRefinance(currentBalance, currentRate, currentTerm, newRate, newTerm, true, refinanceFees)}catch(e){return null}
  },[currentBalance, currentRate, currentTerm, newRate, newTerm, refinanceFees])
    ...
    <CalculatorLayout title="Student Loan Refinance Calculator USA 2026" description="Compare current and proposed student-loan payments and total interest under fixed-rate amortization, with a warning about federal benefits lost in private refinancing." icon="🎓" category="Finance" relatedCalculators={relatedCalculators} slug="student-loan-refinance-calculator">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-1 h-fit space-y-3">
          ...
          <div className="space-y-1">
            <label className="text-xs font-medium text-gray-600">Refinance Fees ($)</label>
            <div className="flex items-center gap-2 border rounded-xl px-3 py-2" style={{background:'rgba(248,250,248,0.8)',borderColor:'rgba(226,232,240,0.7)'}}>
              <span className="text-green-600 text-sm">$</span><input type="number" min={0} value={refinanceFees} onChange={e=>setRefinanceFees(Number(e.target.value))} step={100} className="bg-transparent text-gray-900 font-semibold w-full outline-none text-right" />
            </div>
          </div>
          <div className="space-y-1">
          ...
          {result ? (
            <>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                ...
                <ResultCard label="Break-Even" value={result ? (result.breakEvenMonths ? `${result.breakEvenMonths} months` : "No fee break-even") : "—"} />
              </div>
