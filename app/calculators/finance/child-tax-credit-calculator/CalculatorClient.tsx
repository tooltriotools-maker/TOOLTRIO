...
export default function CalculatorClient({faqs,relatedCalculators}:Props) {
  ...
  const [qualifyingCarePersons, setQualifyingCarePersons] = useState(2)
  const [qualifyingCareExpenses, setQualifyingCareExpenses] = useState(6000)

  const result = useMemo(()=>{
    try{return calculateChildTaxCredit(numChildren, childrenUnder6, agi, 'single', earned, qualifyingCarePersons, qualifyingCareExpenses)}catch(e){return null}
  },[numChildren, childrenUnder6, agi, earned, qualifyingCarePersons, qualifyingCareExpenses])
    ...
    <CalculatorLayout title="Child Tax Credit Calculator USA 2026 — CTC & ACTC" description="Calculate Child Tax Credit, Additional Child Tax Credit refundable portion, and Child & Dependent Care Credit for your family." icon="👶" category="Finance" relatedCalculators={relatedCalculators} slug="child-tax-credit-calculator">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-1 h-fit space-y-3">
          ...
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-2 border rounded-xl px-3 py-2" style={{background:'rgba(248,250,248,0.8)',borderColor:'rgba(226,232,240,0.7)',backdropFilter:'blur(6px)'}}>
              <input type="number" min={0} max={10} value={qualifyingCarePersons} onChange={e=>setQualifyingCarePersons(Math.max(0, Number(e.target.value)))} step={1} className="bg-transparent text-gray-900 font-semibold w-full outline-none text-right" />
            </div>
            <p className="text-[11px] text-gray-500">Use 1 for one qualifying person or 2+ for the $6,000 expense cap.</p>
          </div>
          <div className="space-y-1">
            <label className="text-xs font-medium text-gray-600">Qualifying Care Expenses ($)</label>
            <div className="flex items-center gap-2 border rounded-xl px-3 py-2" style={{background:'rgba(248,250,248,0.8)',borderColor:'rgba(226,232,240,0.7)',backdropFilter:'blur(6px)'}}>
              <span className="text-green-600 text-sm">$</span>
              <input type="number" min={0} value={qualifyingCareExpenses} onChange={e=>setQualifyingCareExpenses(Math.max(0, Number(e.target.value)))} step={500} className="bg-transparent text-gray-900 font-semibold w-full outline-none text-right" />
            </div>
            <p className="text-[11px] text-gray-500">The calculator caps eligible expenses at $3,000 for one person or $6,000 for two or more.</p>
          </div>
        </Card>
          ...
          {result ? (
            <>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                ...
                <ResultCard label="Child Care Credit" value={result ? `${Number(result.childCareCredit).toLocaleString(undefined,{maximumFractionDigits:0})}` : "—"} subValue={result ? `${Math.round(Number(result.childCareRate)*100)}% modeled rate` : undefined} />
      ...
      <div className="mt-8">
        <SEOContent title="Child Tax Credit Calculator USA 2026 — CTC & ACTC" category="finance"
          ...
          howItWorks={'The current code starts with $2,200 per child, applies a $50-per-$1,000 phaseout above $200,000 single or $400,000 married, and estimates refundable ACTC as 15% of earned income above $2,500 capped at $1,700 per child. It then applies the 2026 child-and-dependent-care percentage to the qualifying expenses you enter, subject to the $3,000/$6,000 statutory expense cap. Because the page does not collect filing status beyond a simplified single-filer model, tax liability, provider information, earned-income constraints, dependent-care benefits or SSN eligibility, it cannot reproduce Schedule 8812 or Form 2441.'}
