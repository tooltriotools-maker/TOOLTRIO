'use client'
import { calculateFIRENumber } from '@/lib/calculations/finance'
...
export default function CalculatorClient({faqs,relatedCalculators}:Props) {
  ...
  const [currentPortfolio, setCurrentPortfolio] = useState(180000)
  ...
  const result = useMemo(()=>{
    try{return calculateFIRENumber(annualExpenses, currentAge, targetRetirementAge, currentPortfolio, annualSavings, expectedReturn, safeWithdrawal, 3.0)}catch(e){return null}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-1 h-fit space-y-3">
          ...
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-2 border rounded-xl px-3 py-2" style={{background:'rgba(248,250,248,0.8)',borderColor:'rgba(226,232,240,0.7)',backdropFilter:'blur(6px)'}}>
              <span className="text-green-600 text-sm">$</span>
              <input type="number" value={currentPortfolio} onChange={e=>setCurrentPortfolio(Number(e.target.value))} step={5000} className="bg-transparent text-gray-900 font-semibold w-full outline-none text-right" />
          ...
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-2 border rounded-xl px-3 py-2" style={{background:'rgba(248,250,248,0.8)',borderColor:'rgba(226,232,240,0.7)',backdropFilter:'blur(6px)'}}>
              ...
              <input type="number" value={safeWithdrawal} onChange={e=>setSafeWithdrawal(Number(e.target.value))} step={0.25} className="bg-transparent text-gray-900 font-semibold w-full outline-none text-right" />
              <span className="text-gray-400 text-sm">%</span>
            </div>
          </div>
          <div className="space-y-1">
          ...
          {result ? (
            <>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                <ResultCard label="FIRE Number" value={result ? `${Number(result.fireNumber).toLocaleString(undefined,{maximumFractionDigits:0})}` : "—"} highlight />
                <ResultCard label="Current Portfolio" value={result ? `${Number(result.currentPortfolio).toLocaleString(undefined,{maximumFractionDigits:0})}` : "—"} />
                <ResultCard label="Funding Gap" value={result ? `${Number(result.shortfall).toLocaleString(undefined,{maximumFractionDigits:0})}` : "—"} />
                <ResultCard label="Portfolio Coverage" value={result ? `${Number(result.coveragePercent).toFixed(1)}%` : "—"} />
                <ResultCard label="On Track" value={result ? String(result.onTrack ? '✅ Yes' : '❌ Needs adjustment') : "—"} />
              ...
              <Card>
                ...
                <p className="text-sm text-gray-600 leading-relaxed">Your FIRE number is the portfolio size that generates enough investment returns to fund your lifestyle indefinitely. At a 4% safe withdrawal rate, you need 25x your annual expenses. This calculator shows exactly where you stand, how long until you reach financial independence, and what adjustments would accelerate your timeline.</p>
              </Card>
            </>
          ):(
