...
export const metadata: Metadata = generateCalculatorMetadata({ title: 'Wash Sale Rule Calculator USA 2026 | ToolTrio', description: 'Calculate wash sale rule impact on your tax-loss harvesting. Determine if your trade triggers the 30-day wash sale window and how much loss is disallowed.', slug: 'wash-sale-calculator', category: 'finance', region: 'usa', keywords: ['wash sale calculator USA', 'wash sale rule 30 days', 'tax loss harvesting wash sale', 'disallowed loss calculator'] })
const faqs = [
...
export default function Page() { return <>{structuredData.map((s, i) => <script key={i} type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(s) }} />)}<CalculatorClient faqs={faqs} structuredData={structuredData} relatedCalculators={relatedCalculators} />      <CalculatorBatch43DeepDive slug="wash-sale-calculator" />
</> }
