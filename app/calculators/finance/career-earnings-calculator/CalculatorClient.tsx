...
import { calculateCareerEarningsPotential } from '@/lib/calculations/finance'
...
export default function CalculatorClient({faqs,structuredData,relatedCalculators}:Props){
  ...
  const [promotionFrequencyYears, setPromotionFrequencyYears] = useState(5)
  const [promotionRaisePercent, setPromotionRaisePercent] = useState(12)
  const [taxRate, setTaxRate] = useState(24)
  const result=useMemo(()=>{try{return calculateCareerEarningsPotential(currentAge,currentSalary,retirementAge,annualRaiseRate,promotionFrequencyYears,promotionRaisePercent,taxRate)}catch(e){return null}},[currentAge, currentSalary, retirementAge, annualRaiseRate, promotionFrequencyYears, promotionRaisePercent, taxRate])
  return(
    <CalculatorLayout title="Career Lifetime Earnings Calculator USA 2026" description="Project total lifetime earnings from current salary to retirement including raises and promotions — before and after tax." icon="💹" category="Finance" structuredData={structuredData} relatedCalculators={relatedCalculators} slug="career-earnings-calculator">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-1 h-fit space-y-3">
          ...
          <div className="space-y-1"><label className="text-xs font-medium text-gray-600">Retirement Age</label>
            <div className="flex items-center gap-2 border rounded-xl px-3 py-2" style={{background:'rgba(248,250,248,0.8)',borderColor:'rgba(226,232,240,0.7)'}}>
          ...
          <div className="space-y-1"><label className="text-xs font-medium text-gray-600">Annual Raise (%)</label>
            <div className="flex items-center gap-2 border rounded-xl px-3 py-2" style={{background:'rgba(248,250,248,0.8)',borderColor:'rgba(226,232,240,0.7)'}}>
          ...
          <div className="space-y-1"><label className="text-xs font-medium text-gray-600">Promotion Every N Years</label>
            <div className="flex items-center gap-2 border rounded-xl px-3 py-2" style={{background:'rgba(248,250,248,0.8)',borderColor:'rgba(226,232,240,0.7)'}}>
              
              <input type="number" value={promotionFrequencyYears} onChange={e=>setPromotionFrequencyYears(Number(e.target.value))} step={1} className="bg-transparent text-gray-900 font-semibold w-full outline-none text-right"/>
              <span className="text-gray-400 text-sm">yrs</span>
            </div></div>
          <div className="space-y-1"><label className="text-xs font-medium text-gray-600">Promotion Raise (%)</label>
            <div className="flex items-center gap-2 border rounded-xl px-3 py-2" style={{background:'rgba(248,250,248,0.8)',borderColor:'rgba(226,232,240,0.7)'}}>
              
              <input type="number" value={promotionRaisePercent} onChange={e=>setPromotionRaisePercent(Number(e.target.value))} step={1} className="bg-transparent text-gray-900 font-semibold w-full outline-none text-right"/>
              <span className="text-gray-400 text-sm">%</span>
            </div></div>
        ...
        <div className="lg:col-span-2 space-y-4">
          {result?(<>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                <ResultCard label="Final Salary" value={result?`${Number(result.finalSalary).toLocaleString(undefined,{maximumFractionDigits:0})}`:"-"} highlight/>
                <ResultCard label="Total Lifetime Earnings" value={result?`${Number(result.totalEarnings).toLocaleString(undefined,{maximumFractionDigits:0})}`:"-"}/>
                <ResultCard label="Total After-Tax" value={result?`${Number(result.totalAfterTax).toLocaleString(undefined,{maximumFractionDigits:0})}`:"-"}/>
                <ResultCard label="Total Tax Paid" value={result?`${Number(result.totalTaxPaid).toLocaleString(undefined,{maximumFractionDigits:0})}`:"-"}/>
                <ResultCard label="Avg Annual Earnings" value={result?`${Number(result.avgAnnualEarnings).toLocaleString(undefined,{maximumFractionDigits:0})}`:"-"}/>
                <ResultCard label="Salary Growth" value={result?`${Number(result.salaryGrowth).toFixed(1)}%`:"-"}/>
            </div>
            ...
            <Card><h2 className="text-lg font-black text-gray-900 mb-2">💹 Career Lifetime Earnings Calculator USA 2026</h2><p className="text-sm text-gray-600">Project total lifetime earnings from current salary to retirement including raises and promotions — before and after tax.</p></Card>
