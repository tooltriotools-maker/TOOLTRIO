...
export default function CalculatorClient({faqs,structuredData,relatedCalculators}:Props){
  ...
  return(
    <CalculatorLayout title="Social Security Disability (SSDI) Calculator USA 2026" description="Estimate an educational SSDI PIA from a user-entered AIME estimate and display a separate simplified work-credit screen." icon="🦽" category="Finance" structuredData={structuredData} relatedCalculators={relatedCalculators} slug="ssdi-benefit-calculator">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-1 h-fit space-y-3">
          <h2 className="text-sm font-semibold text-green-600 uppercase tracking-wider">Enter Your Details</h2>
          <div className="space-y-1"><label className="text-xs font-medium text-gray-600">Estimated AIME for SSA Formula ($/mo)</label>
        ...
        <div className="lg:col-span-2 space-y-4">
          {result?(<>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                ...
                <ResultCard label="Eligible" value={result?String(result.eligible ? 'Credit estimate passes simplified screen' : 'Check official SSA credits'):"-"}/>
            ...
            <Card><h2 className="text-lg font-black text-gray-900 mb-2">🦽 Social Security Disability (SSDI) Calculator USA 2026</h2><p className="text-sm text-gray-600">Estimate an educational SSDI PIA from a user-entered AIME estimate and display a separate simplified work-credit screen.</p></Card>
          </>):(<Card><p className="text-gray-500 text-center py-8">Fill in your details to see results →</p></Card>)}
        </div>
      </div>
      <div className="mt-8">
        <SEOContent title="Social Security Disability (SSDI) Calculator USA 2026" category="finance" intro="Estimate a simplified SSDI benefit from a user-entered AIME estimate and see the calculator’s work-credit screen. It is most useful for understanding the PIA formula, not for predicting an SSA award." howItWorks="For a worker becoming eligible in 2026, the code now applies SSA’s 2026 PIA bend points: 90% of the first $1,286 of AIME, 32% from $1,286 through $7,749, and 15% above $7,749. The page also compares estimated credits earned from years worked with a simplified age-based credit requirement." tipsSection="Do not enter ordinary recent wages as though they were automatically AIME; SSA computes AIME from indexed covered earnings under disability-specific rules. SSA indexes covered earnings and applies disability-specific computation rules. The work-credit test here is simplified and does not fully model the recent-work test or every special rule." conclusion="Use this result as an educational estimate only. SSA determines disability status, insured status, AIME and the final PIA from your official earnings record."
