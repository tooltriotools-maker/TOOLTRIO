...
export default function Page() {
  ...
  const c = data['crude_wti' as keyof typeof data]
  const [qty, setQty] = useState(1000)

  const conversions = [{label:"Per Barrel",factor:1,unit:"bbl"},{label:"Per Gallon",factor:1/42,unit:"gal"},{label:"Per Litre",factor:1/158.987,unit:"L"},{label:"Per Metric Ton",factor:1/7.33,unit:"MT"},{label:"Per Mbbl",factor:1000,unit:"Mbbl"},{label:"1000 bbls",factor:1000,unit:"Kbbl"}]
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      <nav className="text-xs text-gray-400 mb-5 flex items-center gap-1">
        ...
        <span className="text-gray-700 font-medium">Crude Oil (WTI) Calculator</span>
      </nav>
      {/* ── PDF Export Buttons ── */}
      <div className="flex flex-wrap items-center gap-2 my-4">
        <ShareButton title="Crude Oil Calculator" category="Commodities" />
          <ExportButtonPair title="Crude Oil Calculator" category="Finance" />
      </div>
      <div className="flex flex-wrap items-start justify-between gap-3 mb-6">
        <div>
          <h1 className="text-3xl font-black text-gray-900" style={{fontFamily:"'Inter', system-ui, sans-serif"}}>🛢️ WTI Crude Oil Price Calculator</h1>
          <p className="text-gray-500 mt-1">Live WTI crude oil price per barrel, gallon, litre and metric ton. West Texas Intermediate - US benchmark.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="space-y-4">
          ...
          <div className="p-4 bg-orange-50 rounded-2xl border border-orange-100 text-xs text-gray-600 leading-relaxed">WTI (West Texas Intermediate) is the US oil benchmark. 1 barrel = 42 US gallons = 158.987 litres. Traded on NYMEX. Usually trades at a slight discount to Brent Crude.</div>
        </div>
